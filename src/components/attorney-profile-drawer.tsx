...
export function AttorneyProfileDrawer({
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-y-0 right-0 z-50 w-full max-w-lg overflow-y-auto bg-surface shadow-xl transition-transform duration-300 ${
          open ? "translate-x-0" : "translate-x-full"
        }`}
      >
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-b-[0.5px] border-border-custom bg-surface px-6 py-4">
        ...
        <div className="px-6 py-6">
          ...
          <p className="text-base text-gold">{attorney.title}</p>
          <p className="mt-1 text-sm text-text-muted">
            {attorney.experience} experience
          </p>

          <div className="mt-6">
            <h4 className="mb-2 text-xs font-semibold uppercase tracking-widest text-text-soft">
              Practice Areas
            </h4>
          ...
          <div className="mt-6">
            ...
            <ul className="space-y-1">
              {attorney.education.map((edu) => (
                <li key={edu} className="text-sm text-text-muted">
                  {edu}
                </li>
              ))}
            </ul>
          ...
          <div className="mt-6">
            ...
            <ul className="space-y-1">
              {attorney.barAdmissions.map((bar) => (
                <li key={bar} className="text-sm text-text-muted">
                  {bar}
                </li>
              ))}
            </ul>
          ...
          <Link
            ...
            className={cn(
              ...
              "mt-8 w-full rounded-xl bg-navy text-center text-white hover:bg-navy-soft"
            )}
          >
